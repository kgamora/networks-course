...
3. Какой IP-адрес вашего компьютера? Какой адрес сервера gaia.cs.umass.edu?
   - (Request) Source Address: 192.168.8.143 (локальный адрес в домашней сети)  
...
5. Когда HTML-файл, который вы извлекаете, последний раз модифицировался на сервере?
   - Last-Modified: Thu, 10 Oct 2024 05:59:02 GMT\r\n
...
2. Проверьте содержимое ответа сервера. Вернул ли сервер содержимое файла явно? Как вы
   это можете увидеть?
   - Да, вернул: тело ответа содержит файл
   - Это можно увидеть, посмотрев в конец пакета: там в явном виде всё содержимое страницы
3. Теперь проверьте содержимое второго HTTP-запроса GET (из вашего браузера на сторону
   ...
   - Да, в запросе есть `If-Modified-Since: Thu, 10 Oct 2024 05:59:02 GMT\r\n`
   - Эта дата - это дата модификации файла из первого ответа сервера: `Last-Modified: Thu, 10 Oct 2024 05:59:02 GMT\r\n`
4. Какой код состояния HTTP и фраза возвращаются сервером в ответ на этот второй запрос
   ...
   - 304 Not Modified
   - Нет, не вернул: у ответа нет (пустое, \r\n) тела
...
1. Сколько сообщений HTTP GET отправил ваш браузер? Какой номер пакета в трассировке
   ...
   - 2: за телом страницы и за favicon
   - 11, 16
...
1. Сколько HTTP GET запросов было отправлено вашим браузером? На какие интернет-адреса были отправлены эти GET-запросы?
   - 3
   - <host>/wireshark-labs/HTTP-wireshark-file4.html, <host>/pearson.png, <host>/8E_cover_small.jpg
2. Можете ли вы сказать, загрузил ли ваш браузер два изображения последовательно или
   ...
   - Последовательно. Ответ на запрос первой картинки пришёл через .2 секунды, запрос на вторую через .35 секунд. Следовательно, мы дожидались ответа
...
2. Когда ваш браузер отправляет сообщение HTTP GET во второй раз, какое новое поле включается в сообщение HTTP GET?
   ...
   > Это значит, что используется базовая схема авторизации, а строка - base 64 encoded строка логин:пароль:
   `echo 'd2lyZXNoYXJrLXN0dWRlbnRzOm5ldHdvcms' | base64 -d
